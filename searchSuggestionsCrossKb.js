$(document).ready(function() {mediaWiki.loader.using('jquery.suggestions', function () {
	$('#searchInput, #searchInput2, #powerSearchText, #searchText').suggestions({
			...
			var request = $.ajax({url:wgScriptPath+'/api.php',
				...
				success: function(data){
					if (data && 1 in data) {
						...
						if (data[1].length < 8) {
							var orig = data[1], alt = '', lq = query.toLowerCase(), hes = "qwertyuiopasdfghjkl;zxcvbnm,./'קראטוןםפשדגכעיחלךףזסבהנמצתץ";
		...
		result: {select: function ($input) {$input.closest('form').submit();}},
		delay: 120,
		positionFromLeft: $('body').is('.rtl'),
		highlightInput:true
	}).bind('paste cut drop input', function(e) {$(this).trigger('keypress');});
});});