/*Author:[[user:שמוליק]]*/
function between(source, before, lb, ob, after, la){
	var i;
	if (before) {
		i = ((lb ? source.indexOf(before) : source.lastIndexOf(before))+ 1) || return '';
		source = source.substring(i + ob);
	}
	if (after) {
		var i = ((la ? source.lastIndexof(after) : source.indexOf(after)) + 1) || return '';
		source = source.substring(0, source.length-i);
	}
	return source;
}
function wikiit() {
  ...
  switch (location.hostname)
  {
    case "www.ynet.co.il":
      ...
      date = between(h1.parent().find(".text12g").last().html(), null, ',').split('.');
      url = between(location.href, 'L-', false, 1, ',');
      template = "ynet";
      break;
    case "news.walla.co.il":
      ...
      date = between(h1.parent().children().eq(2).html(), ",", false, ",", true);
      misc = between(url, null, 0, "/", true);
      misc = "|"+between(misc, "/", true);
      url = between(url, "/", true);
      template = "וואלה!";
      break;
    case "www.nrg.co.il":
      title = $("h1").first().html();
      if (title!=null)
      {
        ...
        author = between(acb, "<!-- ARTICLE_WRITER_START --> ", false, 0, "<!-- ARTICLE_WRITER_END -->");
        date = between(acb, "<!-- ARTICLE_DATE_START -->", false, 0, "<!-- ARTICLE_DATE_END -->");
  ...
  if (date instanceof Array)
  {
    ...
    else if (Number(date[2])>=50&&Number(date[2])<1900) date[2]=19+date[2];
    date =date[0]+" ב"+date[1]+" "+date[2];
  }
  date = jQuery.trim(date);
  author = author.replace(/<a .*?>/g,'');
  author = jQuery.trim(author.replace(/<\/a>/g,''));
  prompt("Your requested template", "{{"+template+"|"+author+"|"+title+"|"+url+"|"+date+"|"+misc+"}}");
}
(function ()
{
