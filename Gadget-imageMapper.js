...
var IME_TRANSLATIONS = 'http://toolserver.org/~dapete/ime/translations.php?lang=' + wgUserLanguage;
...
function ime_init1() {
	...
	$.get(url, function(data) {
		if (data && data.query && data.query.pages && data.query.pages.length) {
			var dim = data.query.pages[0].imageinfo;
			ime_width = dim.width;
			ime_height = dim.height;
...
function ime_init2() {
	...
	img.id = 'imeImg';
	$.extend(img.style, {zIndex: 99, border: 'none', opacity: '0.75', filter: 'alpha(opacity=75)'/*ie*/}); 
	...
	$.extend(divImePreview.style, {position: 'absolute', top: '0', left: '0', width: img.width + 'px', height: img.height + 'px'}); 
...
*/
function ime_CircleCoord(x,y,r) {$.extend(this, {x: x, y: y, radius: r});}
...
function ime_Area(shape) {
	var coords = (shape=='rect') ? new ime_RectCoord(0,0,0,0) : (shape=='circle') ? new ime_CircleCoord(0,0,20) : new ime_PolyCoord();
	$.extend(this, {shape: shape || 'poly', coords: coords, link: '', title: ''});
}
...
function ime_eventGetX(e) { return Math.round((e.layerX || e.offsetX || e.x) / ime_scale); }

function ime_eventGetY(e) { return Math.round((e.layerY || e.offsetY || e.y) / ime_scale); }

function ime_eventGetButton(e) {return e.which || e.button;}

function ime_mouseEventClear() {ime_mouseEventSet(null, '');}

function ime_mouseEventSet(func, cursor) {
	var img = document.getElementById('imeImg');
	img.onmousedown = func;
	img.style.cursor = (typeof cursor == 'undefined' ? 'crosshair' : cursor);
}
